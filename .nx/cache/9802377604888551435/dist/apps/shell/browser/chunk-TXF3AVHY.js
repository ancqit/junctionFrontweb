...
function createExposeModuleLoader(config, ports) {
  const loadRemoteModule2 = (remoteName, exposedModule) => {
    try {
  ...
  return () => Promise.resolve(loadRemoteModule2);
}
...
var initFederation = (remotesOrManifestUrl, options = {}) => {
  ...
  return initFlow(remotesOrManifestUrl).then(
    ({ loadRemoteModule: loadRemoteModule2 }) => createFederationResult({ config, adapters, loadRemoteModule: loadRemoteModule2, initRemoteEntryFlow })
  ).catch((e) => {
...
async function resolveRemoteNameFromEntry(remoteEntry) {
  ...
  if (!res.ok) {
    throw new Error(`Failed to fetch remoteEntry at ${remoteEntry}: ${res.status} ${res.statusText}`);
  }
...
async function loadRemoteModule(optionsOrRemoteName, exposedModule) {
  ...
  try {
    ...
    if (options.remoteEntry) {
      federation = await federation.initRemoteEntry(options.remoteEntry, options.remoteName);
    }
    ...
    return await federation.loadRemoteModule(options.remoteName, options.exposedModule);
  } catch (err) {
    if (fallback) {
...
export {
  __spreadValues,
  __spreadProps,
  initFederation2 as initFederation,
  loadRemoteModule
};
