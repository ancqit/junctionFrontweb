...
(function() {
  ...
  const initHotReload = (topLevelLoad2, importShim2) => {
    ...
    const Hot = class Hot {
      ...
      accept(deps, cb) {
        ...
        (hotState.a = hotState.a || []).push([
          typeof deps === "string" ? defaultResolve2(deps, this.url) : deps ? deps.map((d) => defaultResolve2(d, this.url)) : null,
          cb
        ]);
      }
      dispose(cb) {
    ...
    const handleInvalidations = () => {
      ...
      for (const root of curInvalidationRoots) {
        ...
        topLevelLoad2(
          ...
          baseUrl,
          defaultFetchOpts,
          typeof hotState.e === "string" ? hotState.e : void 0,
          false,
          void 0,
          hotState.t
        ).then((m) => {
          ...
          hotState.p.forEach((p2) => {
            ...
            if (hotState2 && hotState2.a)
              hotState2.a.forEach(
                async ([d, c2]) => d && !earlyRoots.has(c2) && (typeof d === "string" ? d === root && c2(m) : c2(await Promise.all(d.map((d2) => (earlyRoots.add(c2), importShim2(toVersioned(d2, getHotState(d2))))))))
              );
          });
        }, throwError);
      }
    ...
    setHooks(
      ...
      _resolveHook ? (id, parent, defaultResolve3) => hotResolveHook(id, parent, (id2, parent2) => _resolveHook(id2, parent2, defaultResolve3)) : hotResolveHook,
  ...
  const defineValue = (obj, prop, value) => Object.defineProperty(obj, prop, { writable: false, configurable: false, value });
    ...
    if (!hasDocument)
      return Promise.all([
        import(createBlob(`import"${createBlob("{}", "text/json")}"with{type:"json"}`)).then(
          () => (supportsJsonType = true, import(createBlob(`import"${createBlob("", "text/css")}"with{type:"css"}`)).then(
            () => supportsCssType = true,
            noop
          )),
          noop
        ),
        wasmInstancePhaseEnabled && import(createBlob(`import"${createBlob(new Uint8Array(wasmBytes), "application/wasm")}"`)).then(
          () => supportsWasmInstancePhase = true,
          noop
        ),
        wasmSourcePhaseEnabled && import(createBlob(`import source x from"${createBlob(new Uint8Array(wasmBytes), "application/wasm")}"`)).then(
  ...
  function parse(k2, l2 = "@") {
      ...
      a = new ArrayBuffer(i), s(f, new Uint16Array(a, 16, 108)), e = (function(e2, a2, r2) {
        ...
        function b2(e3) {
          ...
          e: while (1) {
            ...
            r3 = f2[68] | 0;
            a: do {
              if ((r3 | 0) == (p3 | 0)) if (g2 & ((s2[398] | 0) == y2 << 16 >> 16 & e3)) {
                a3 = s2[f2[71] >> 1] | 0;
                if (se(a3) | 0) break e;
                else a3 = 1;
              } else a3 = g2;
        ...
        return { su: le, ai: X, e: ke, ee: ie, ele: R, els: W, es: ce, id: J, ie: Y, ip: te, is: fe, it: be, p: k3, re: _, ri: Z, sa: j, se: V, ses: ne, ss: re };
    ...
    for (; e.ri(); ) {
      ...
      let l3 = null;
      w.push({ t: n2, n: k3, s: a2, e: r2, ss: f2, se: t2, d: s2, a: i2, at: l3 });
    }
    for (; e.re(); ) {
      const a2 = e.es(), r2 = e.ee(), i2 = e.els(), s2 = e.ele(), f2 = i2 < 0 ? void 0 : v(i2, s2), c2 = v(a2, r2);
      d.push({ s: a2, e: r2, ls: i2, le: s2, n: c2, ln: f2 });
    }
    return [w, d];
    function v(e2, a2) {
  ...
  const resolveDeps = (load, seen) => {
    ...
    resolvedSource = "";
    lastIndex = 0;
    for (const { s: start, e: end, ss: statementStart, se: statementEnd, d: dynamicImportIndex, t: t2, a: a2, at } of imports) {
      if (t2 === 4) {
        ...
        if (cycleShell) {
          if (!(blobUrl = depLoad.s)) {
            blobUrl = depLoad.s = createBlob(
              `export function u$_(m){${depLoad.a[1].map(({ s: s2, e: e2 }, i2) => {
...
` + e2.message;
  ...
  async function defaultSourceHook(url, fetchOpts, parent) {
    let res = await doFetch(url, fetchOpts, parent), contentType, [, json, type, jsts] = (contentType = res.headers.get("content-type") || "").match(
...
`;
  ...
  const linkLoad = (load, fetchOpts) => {
    ...
    load.L = load.f.then(() => {
      ...
      load.d = load.a[0].map(({ n: n2, d, t: t2, a: a2, se }) => {
        ...
        if (childFetchOpts.integrity) childFetchOpts = { ...childFetchOpts, integrity: void 0 };
    ...
    if (ts && !script.src) {
      loadPromise = Promise.resolve(esmsTsTransform || initTs()).then(() => {
        ...
        return topLevelLoad(
          ...
          baseUrl,
          getFetchOpts(script),
          transformed === void 0 ? script.innerHTML : transformed,
          !shimMode && transformed === void 0,
      ...
      loadPromise = topLevelLoad(
        ...
        baseUrl,
        getFetchOpts(script),
        !script.src ? script.innerHTML : void 0,
        !shimMode,
        isBlockingReadyScript && lastStaticLoadPromise,
        ts ? "ts" : void 0
      ).catch(throwError);
